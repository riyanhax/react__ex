...
export default class ExForm extends React.Component {
  ...
  render() {
    ...
    const { action, limitFormData, onSubmit, onChange, quotePair, basePair } = this.props;
      ...
      if (name == "proc") {
        ...
        formFields.push(
          <div key={name} className="ex__form__item fb gray__text">
            <label htmlFor={`ex__form__buy__${item.name}`} className="ex__form__label">{name}:</label>
            <input type="number"
              id={`ex__form__buy__${item.name}`}
              name={name}
              value={item.value}
              step="0.01"
              min="0.01"
